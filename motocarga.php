...
redirectIfNotLogged("motocarga.php");
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $motocargaId = intval($_GET['id']);
    $sqlMotocarga = "SELECT * FROM motocarga WHERE id = $motocargaId LIMIT 1";
    $resultMotocarga = mysqli_query($conn, $sqlMotocarga);
    if ($resultMotocarga && mysqli_num_rows($resultMotocarga) > 0) {
        $editMode = true;
        $motocarga = mysqli_fetch_assoc($resultMotocarga);
        $nombre = $motocarga['nombre'];
        $apellido = $motocarga['apellido'];
        $dni = $motocarga['dni'];
        $direccion = $motocarga['direccion'];
        $telefono = $motocarga['telefono'];
        $estado = $motocarga['estado'];
        $latitud = $motocarga['latitud'];
        $longitud = $motocarga['longitud'];
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $motocargaId = isset($_POST['id']) && is_numeric($_POST['id']) ? intval($_POST['id']) : null;
    ...
    if ($motocargaId) {
        $checkSql = "SELECT id FROM motocarga WHERE dni = '$dni' AND id != $motocargaId LIMIT 1";
        $checkRes = mysqli_query($conn, $checkSql);

        if ($checkRes && mysqli_num_rows($checkRes) > 0) {
            $mensaje = "El DNI ya existe en la base de datos.";
        } else {
            if (actualizarMotoCarga($conn, $motocargaId, $nombre, $apellido, $dni, $direccion, $telefono, $estado, $latitud, $longitud)) {
                header("Location: listar_motocarga.php");
                exit();
            } else {
                $mensaje = "Error al actualizar la motocarga: " . mysqli_error($conn);
            }
        }
    } else {
        $checkSql = "SELECT id FROM motocarga WHERE dni = '$dni' LIMIT 1";
        $checkRes = mysqli_query($conn, $checkSql);

        if ($checkRes && mysqli_num_rows($checkRes) > 0) {
            $mensaje = "El DNI ya existe en la base de datos.";
        } else {
            if (crearMotoCarga($conn, $nombre, $apellido, $dni, $direccion, $telefono, $estado, $latitud, $longitud)) {
                header("Location: listar_motocarga.php");
                exit();
            } else {
                $mensaje = "Error al crear la motocarga: " . mysqli_error($conn);
            }
        }
    }
}
...
function crearMotoCarga($conn, $nombre, $apellido, $dni, $direccion, $telefono, $estado, $latitud = '', $longitud = '')
{
    $latitudValue = $latitud === '' ? 'NULL' : "'$latitud'";
    $longitudValue = $longitud === '' ? 'NULL' : "'$longitud'";
    $sql = "INSERT INTO motocarga (nombre, apellido, dni, direccion, telefono, estado, latitud, longitud) 
    VALUES ('$nombre', '$apellido', '$dni', '$direccion', '$telefono', '$estado', $latitudValue, $longitudValue)";
    return mysqli_query($conn, $sql);
}

function actualizarMotoCarga($conn, $id, $nombre, $apellido, $dni, $direccion, $telefono, $estado, $latitud = '', $longitud = '')
{
    $latitudValue = $latitud === '' ? 'NULL' : "'$latitud'";
    $longitudValue = $longitud === '' ? 'NULL' : "'$longitud'";
    $sql = "UPDATE motocarga SET nombre = '$nombre', apellido = '$apellido', dni = '$dni', direccion = '$direccion', telefono = '$telefono', estado = '$estado', latitud = $latitudValue, longitud = $longitudValue WHERE id = $id";
    return mysqli_query($conn, $sql);
}

function obtenerMotoCargas($conn)
{
    $sql = "SELECT * FROM motocarga";
    return mysqli_query($conn, $sql);
}
?>
...
<div class="min-vh-100 d-flex flex-column justify-content-start align-items-center py-4" 
    ...
    <div class="container-fluid px-4 px-md-5">
        <!-- HEADER -->
        <header class="motocarga-header mb-5">
            <div style="width: 100%;">
                <nav class="motocarga-menu d-flex justify-content-end align-items-center gap-3">
        ...
        <div class="row g-5 align-items-center">
            ...
            <div class="col-lg-6 col-12 mb-4 mb-lg-0">
                <div class="motocarga-title">
                    <h2 class="mb-4">
                        ESCOGISTE <br>
                        <span>LA MOTOCARGA</span>
                    </h2>
                </div>
                ...
                <div class="services-grid">
                    <div class="service-item">
                        ...
                        <p>Transporte de materiales de construcción</p>
                    </div>
                    <div class="service-item">
                        <i class="fa-solid fa-person"></i>
                        <p>Mudanzas pequeñas</p>
                    </div>
                    <div class="service-item">
                        <i class="fa-solid fa-truck-fast"></i>
                        <p>Recojo y distribución de mercadería</p>
                    </div>
                    <div class="service-item">
                        <i class="fa-solid fa-store"></i>
                        <p>Delivery para negocios</p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6 col-12">
                <div class="form-section">
                    <h1>
                        <?php echo $editMode ? 'EDITAR MOTOCARGA' : 'INGRESE SUS DATOS'; ?>
                    </h1>
                    ...
                    <form action="motocarga.php" method="POST" novalidate>
                        <?php if ($editMode): ?>
                            <input type="hidden" name="id" value="<?php echo htmlspecialchars($motocargaId); ?>">
                        <?php endif; ?>
                    ...
                    <div class="text-center">
                        <a href="listar_motocarga.php" class="btn btn-warning btn-sm fw-bold px-4">
                            </i>Ver lista
                        </a>
                    </div>
                </div>
